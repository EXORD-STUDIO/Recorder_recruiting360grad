<script lang="ts">
  ...
  async function versionDialog() {
    console.log(window.electron.process.versions)
    dialogs.add({
      title: 'Version',
      message: `Applikation: ${await window.api.getVersion()} <br> Electron ${window.electron.process.versions.electron} <br> Node.js ${window.electron.process.versions.node} <br> Chrome ${window.electron.process.versions.chrome}`
    })
  }
</script>
